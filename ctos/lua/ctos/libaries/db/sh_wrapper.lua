--[[
    ...
    Yoinked from https://github.com/alexgrist/GLua-MySQL-Wrapper/blob/master/msql_wrapper.lua
    Thanks for the module! :D
--]]

msql_wrapper = msql_wrapper or {
	module = "sqlite"
}
...
local MODULE_NOT_EXIST = "[msql_wrapper] The %s module does not exist!\n"
...
function QUERY_CLASS:Escape(text)
	return msql_wrapper:Escape(tostring(text))
end
...
local function ApplyQueryReplacements(mode, query)
	if (!Replacements[msql_wrapper.module]) then
		return query
	end

	local result = query
	local entries = Replacements[msql_wrapper.module][mode]
...
local function BuildSelectQuery(queryObj)
	...
	if (isstring(queryObj.tableName)) then
		...
		ErrorNoHalt("[msql_wrapper] No table name specified!\n")
		return
	end
...
local function BuildInsertQuery(queryObj, bIgnore)
	local suffix = (bIgnore and (msql_wrapper.module == "sqlite" and "INSERT OR IGNORE INTO" or "INSERT IGNORE INTO") or "INSERT INTO")
...
local function BuildCreateQuery(queryObj)
	...
	if (istable(queryObj.createList) and #queryObj.createList > 0) then
		...
		for i = 1, #queryObj.createList do
			if (msql_wrapper.module == "sqlite") then
...
local function BuildAlterQuery(queryObj)
	...
	elseif (isstring(queryObj.drop)) then
		if (msql_wrapper.module == "sqlite") then
			ErrorNoHalt("[msql_wrapper] Cannot drop columns in sqlite!\n")
			return
		end
...
function QUERY_CLASS:Execute(bQueueQuery)
	...
	if (isstring(queryString)) then
		if (!bQueueQuery) then
			return msql_wrapper:RawQuery(queryString, self.callback)
		else
			return msql_wrapper:Queue(queryString, self.callback)
		end
	end
end
...
function msql_wrapper:Select(tableName)
	return QUERY_CLASS:New(tableName, "SELECT")
end

function msql_wrapper:Insert(tableName)
	return QUERY_CLASS:New(tableName, "INSERT")
end

function msql_wrapper:InsertIgnore(tableName)
	return QUERY_CLASS:New(tableName, "INSERT IGNORE")
end

function msql_wrapper:Update(tableName)
	return QUERY_CLASS:New(tableName, "UPDATE")
end

function msql_wrapper:Delete(tableName)
	return QUERY_CLASS:New(tableName, "DELETE")
end

function msql_wrapper:Drop(tableName)
	return QUERY_CLASS:New(tableName, "DROP")
end

function msql_wrapper:Truncate(tableName)
	return QUERY_CLASS:New(tableName, "TRUNCATE")
end

function msql_wrapper:Create(tableName)
	return QUERY_CLASS:New(tableName, "CREATE")
end

function msql_wrapper:Alter(tableName)
...
function msql_wrapper:Connect(host, username, password, database, port, socket, flags)
	port = port or 3306
	...
	if (self.module == "mysqloo") then
		...
		if (mysqloo) then
			...
			self.connection.onConnected = function(connection)
				...
				msql_wrapper:OnConnected()
			end

			self.connection.onConnectionFailed = function(database, errorText)
				msql_wrapper:OnConnectionFailed(errorText)
			end

			self.connection:connect()

			timer.Create("msql_wrapper.KeepAlive", 300, 0, function()
...
function msql_wrapper:RawQuery(query, callback, flags, ...)
	if (self.module == "mysqloo") then
		...
		queryObj.onSuccess = function(queryObj, result)
			if (callback) then
				...
				if (!bStatus) then
					error(string.format("[msql_wrapper] MySQL Callback Error!\n%s\n", value))
				end
			end
		end

		queryObj.onError = function(queryObj, errorText)
			ErrorNoHalt(string.format("[msql_wrapper] MySQL Query Error!\nQuery: %s\n%s\n", query, errorText))
		end
	...
	elseif (self.module == "sqlite") then
		...
		if (result == false) then
			error(string.format("[msql_wrapper] SQL Query Error!\nQuery: %s\n%s\n", query, sql.LastError()))
		else
			if (callback) then
				local bStatus, value = pcall(callback, result, true, tonumber(sql.QueryValue("SELECT last_insert_rowid()")))

				if (!bStatus) then
					error(string.format("[msql_wrapper] SQL Callback Error!\n%s\n", value))
				end
			end
		end
	else
		ErrorNoHalt(string.format("[msql_wrapper] Unsupported module \"%s\"!\n", self.module))
	end
end

-- A function to add a query to the queue.
function msql_wrapper:Queue(queryString, callback)
	if (isstring(queryString)) then
		QueueTable[#QueueTable + 1] = {queryString, callback}
	end
end

-- A function to escape a string for msql_wrapper.
function msql_wrapper:Escape(text)
...
function msql_wrapper:Disconnect()
	if (self.connection) then
		if (self.module == "mysqloo") then
...
function msql_wrapper:SetModule(moduleName)
	self.module = moduleName
end

-- Called when the database connects sucessfully.
function msql_wrapper:OnConnected()
	MsgC(Color(25, 235, 25), "[msql_wrapper] Connected to the database!\n")

	hook.Run("DatabaseConnected")
end

-- Called when the database connection fails.
function msql_wrapper:OnConnectionFailed(errorText)
	ErrorNoHalt(string.format("[msql_wrapper] Unable to connect to the database!\n%s\n", errorText))

	hook.Run("DatabaseConnectionFailed", errorText)
end

-- A function to check whether or not the module is connected to a database.
function msql_wrapper:IsConnected()
